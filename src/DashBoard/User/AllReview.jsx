  ...
  <table className="table">
    ...
    <tbody>
      {reviews.map((review,idx)=><tr key={review._id}>
        ...
        <td>{review?.review}</td>
        <th>
          <button onClick={()=>handleDelete(review?._id)} className="btn  bg-[#f76042] text-white"><FaTrash></FaTrash></button>
        </th>
        <th>
       <Link to={`/detail/${review?.detailId}`}>  
        <button  className="btn  bg-[#f76042] text-white"><FaEye></FaEye></button></Link>
        </th>
      </tr>)}
